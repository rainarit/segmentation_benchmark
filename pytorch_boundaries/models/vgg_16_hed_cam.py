...
class VGG_HED_CAM(nn.Module):
  def __init__(self, config):
    super(VGG_HED_CAM, self).__init__()
    ...
    self.cam_conv1 = nn.Conv2d(1, 64, 3, padding=1)
    self.cam_pool1 = nn.MaxPool2d(2, 2)

    self.cam_conv2 = nn.Conv2d(64, 128, 3, padding=1)
    self.cam_pool2 = nn.MaxPool2d(2, 2)
    
    self.cam_conv3 = nn.Conv2d(128, 256, 3, padding=1)

    init_conv_layers = [self.dsn1, self.dsn2, self.dsn3, 
                        self.dsn4, self.dsn5,
                        ]
    cam_conv_layers = [self.cam_conv1, self.cam_conv2,
                        self.cam_conv3]

    for layer in init_conv_layers:
      layer.apply(conv_weights_init)

    for layer in cam_conv_layers:
      layer.apply(normal_weights_init)
  ...
  def forward(self, inputs, cam_maps):
    """Forward pass with input images and CAM maps."""
    _, _, h, w = inputs.shape
    net = self.standardize(inputs)
    net = self.first_padding(net)
    cam_net = self.first_padding(cam_maps)

    net = self.conv_1(net)
    cam_net = F.relu(self.cam_conv1(cam_net))
    net = net * cam_net
    cam_net = self.cam_pool1(cam_net)
    self.side_output_1 = crop_tensor(self.dsn1(net), h, w)

    net = self.conv_2(net)
    cam_net = F.relu(self.cam_conv2(cam_net))
    net = net * cam_net
    cam_net = self.cam_pool2(cam_net)
    self.side_output_2 = crop_tensor(
                          self.dsn2_up(self.dsn2(net)), 
                          h, w)

    net = self.conv_3(net)
    cam_net = F.relu(self.cam_conv3(cam_net))
    net = net * cam_net
  ...
  def extract_layer(self, model, backbone_mode, ind):
    if backbone_mode == 'vgg16' or backbone_mode == 'vgg16_cam':
        index_dict = {
            1: (0,4), 
            2: (4,9), 
            3: (9,16), 
            4: (16,23),
            5: (23,30)}
    elif backbone_mode=='vgg16_bn' or backbone_mode == 'vgg16_bn_cam':
        index_dict = {
            1: (0,6), 
            2: (6,13), 
            3: (13,23), 
