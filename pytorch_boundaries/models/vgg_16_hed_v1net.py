...
from pytorch_boundaries.utils.model_utils import (crop_tensor,
...
class VGG_V1Net_HED(nn.Module):
  def __init__(self, config):
    super(VGG_V1Net_HED, self).__init__()
    ...
    self.conv_1 = self.extract_layer(model, 
                                     self.model_name, 
                                     1)
    self.conv1_v1net = V1Net(64, 64, 3, 7, 3)

    self.conv_2 = self.extract_layer(model, 
                                     self.model_name, 
                                     2)
    self.conv2_v1net = V1Net(128, 128, 3, 5, 3)

    self.conv_3 = self.extract_layer(model, 
                                     self.model_name, 
                                     3)
    self.conv3_v1net = V1Net(256, 256, 3, 5, 3)
    
    self.conv_4 = self.extract_layer(model, 
                                     self.model_name, 
                                     4)
    self.conv4_v1net = V1Net(512, 512, 1, 3, 1)
    self.conv_5 = self.extract_layer(model, 
                                     self.model_name, 
                                     5)
    self.conv5_v1net = V1Net(512, 512, 1, 3, 1)
  ...
  def forward(self, inputs):
    ...
    # Apply V1Net recurrence after convolution layer, before DSN layer
    _, (net, _) = self.conv2_v1net(tile_tensor(self.conv_2(net), 4))
    self.side_output_2 = crop_tensor(
                          self.dsn2_up(self.dsn2(net)),
                          h, w)

    _, (net, _) = self.conv3_v1net(tile_tensor(self.conv_3(net), 3))
    self.side_output_3 = crop_tensor(
                          self.dsn3_up(self.dsn3(net)),
                          h, w)

    net = self.conv_4(net)
    # _, (net, _) = self.conv4_v1net(tile_tensor(net, 2))
    self.side_output_4 = crop_tensor(
                          self.dsn4_up(
                              self.dsn4(net)),
                          h, w)
    net = self.conv_5(net)
    # _, (net, _) = self.conv5_v1net(tile_tensor(net, 2))
  ...
  def extract_layer(self, model, backbone_mode, ind):
    if backbone_mode=='vgg16' or backbone_mode == 'vgg16_v1net':
        index_dict = {
            1: (0,4), 
            2: (4,9), 
            3: (9,16), 
            4: (16,23),
            5: (23,30)}
    elif backbone_mode=='vgg16_bn' or backbone_mode == 'vgg16_bn_v1net':
        index_dict = {
            1: (0,6), 
            2: (6,13), 
            3: (13,23), 
