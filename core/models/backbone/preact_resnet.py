...
class PreActResNetV1Net(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10,
                 kernel_size=3, kernel_size_exc=5,
                 kernel_size_inh=3, timesteps=5):
        super(PreActResNetV1Net, self).__init__()
        self.in_planes = 64
    ...
    def forward(self, x):
        out = self.conv1(x)
        n, c, h, w = out.shape
        net_tiled = out.detach().repeat(self.timesteps, 1, 1, 1,
                                        1).view(self.timesteps, n, c, h, w)
        net_tiled = torch.transpose(net_tiled, 1, 0)
        _, (out, _) = self.v1net_conv(net_tiled)
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
...
def PreActResNetV1Net18(num_classes=10, kernel_size=3, kernel_size_exc=5,
                        kernel_size_inh=3, timesteps=5):
    return PreActResNetV1Net(PreActBlock, [2, 2, 2, 2], num_classes,
