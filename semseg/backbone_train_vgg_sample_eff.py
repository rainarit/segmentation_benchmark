...
parser.add_argument('--train-percent', default=25, type=int,
...
def main():
    ...
    dir_idx = 0
    while True:
        dirname = "sample_eff_a_%s_tp_%s_%s" % (args.arch, args.train_percent, dir_idx)
        checkpoint_dir = "checkpoints_%s/%s" % (str(args.data).split("/")[-1], dirname)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    if 'imagenet_100' in str(args.data):
        num_classes = 100
    else:
        num_classes = 1000
    
    train_fns_file = 'train/train_percent_%s_seed_567.txt' % args.train_percent
    with open(args.data / train_fns_file, 'r') as f:
