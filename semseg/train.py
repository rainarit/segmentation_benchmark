...
def evaluate(model, data_loader, device, num_classes, iterator):
    ...
    with torch.no_grad():
        if args.distributed == True:
            Parallel(n_jobs=4)(delayed(distributed_eval)(idx, image, target, model, confmat, data_loader, iterator) for idx, (image, target) in enumerate(metric_logger.log_every(data_loader, 1, header)))
